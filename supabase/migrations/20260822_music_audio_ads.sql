...
create table if not exists public.music_ad_impressions (
    ...
    -- Daily salted listener identity derived by the Edge Function. Raw IP
    -- addresses are never stored, but repeated submissions can be rate-limited.
...
create index if not exists music_ad_impressions_listener_idx
    on public.music_ad_impressions (listener_hash, created_at desc);

create unique index if not exists music_ad_impressions_ad_session_unique
    on public.music_ad_impressions (ad_id, session_id);
...
-- Impression writes go exclusively through the apollo-music Edge Function. It
-- validates the ad and duration, derives identity and completion state, and
-- rate-limits before inserting. Anonymous listening remains supported without
-- allowing clients to manufacture billable events directly.
